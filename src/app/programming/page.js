...
export default async function CodingPage({ searchParams }) {
  const params = await searchParams;
  const status = params.status;

  let query = supabase.schema("habit_tracker").from("code_details").select("*");
    ...
    <main className="p-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mt-8 mb-12 gap-4">
        <div>
          ...
          <p className="opacity-60 mt-2">Tracking Every Program</p>
        </div>
        <ProjectFilter />
      </div>
